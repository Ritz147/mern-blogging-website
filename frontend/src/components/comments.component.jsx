...
export const fetchComments= async ({ skip=0 , blog_id , setParentCommentCount , commentArr=null })=>{
   let res;
    ...
    return(
       <div className={"max-sm:w-full fixed "+(commentWrapper ? "top-0 sm:right-0" : "top-[100%] sm:right-[-100%]" )   +" duration-700 max-sm:right-0 sm:top-0 w-[30%] min-w-[350px] h-full z-50 bg-white shadow-2xl p-8 px-16 overflow-y-auto overflow-x-hidden"}>
         <div className="relative">
            ...
            <button onClick={()=>setCommentWrapper(preVal=>!preVal)} className="absolute top-2 right-0 flex justify-center items-center w-12 h-12 rounded-full bg-grey">
